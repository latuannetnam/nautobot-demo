...
def transposer(data):
    """Some."""
    logger.info("Transposer")
    try:
        if len(data.get("location", {}).get("vlans", [])) > 0:
            data["vlans_merged_list"] = vlan_parser(
                [vlan["vid"] for vlan in data.get("location", {}).get("vlans", [])]
            )
        if data.get("platform", {}).get("name") == "Cisco IOS":
            expand_prefix(data["interfaces"])
        if data.get("platform", {}).get("name") == "Cisco NX-OS":
